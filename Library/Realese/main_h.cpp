...
void return_book_lib (){
	...
	do
	{
		...
		read_book.open("Reader.bin", std::ios::trunc); 	// очистил файл
		read_book.close();
		if (arr_r[numb_r-1].library[0].Flag_b == 1 || arr_r[numb_r-1].library[1].Flag_b == 1 || arr_r[numb_r-1].library[2].Flag_b == 1) 		// если пользователь есть, открой его и верни книгу в библиотеку
		{
			std::cout << "input Id_Book for return: ";
			std::cin >> numb_b;
			for (int i=0, j = 1; i<j; i++)
					{
						if (arr_r[numb_r-1].Hist_book[i].Flag_b == 0)
						{
							arr_r[numb_r-1].Hist_book[i] = arr_b[numb_b-1];
							//std::cerr << "Hist\n";
							
						}
						if (arr_r[numb_r-1].Hist_book[i].Flag_b == 1)
						{
							j++;
							//std::cerr << "Break\n";
						}
			...
			for (int i = 0; i < 3; i++)
			{
				if (arr_r[numb_r-1].library[i].Id_b == numb_b)
				{
					...
					arr_r[numb_r-1].library[i].Flag_b = 0; 	// удаляет книгу из ячейки читателя
				...
				if (arr_r[numb_r-1].library[i+1].Id_b == numb_b)
				{
					...
					arr_r[numb_r-1].library[i+1].Flag_b = 0; 	// удаляет книгу из ячейки читателя
					arr_b[numb_b-1].Flag_b = 1;
					//std::cerr << "Break\n";
					break;
				}
				if (arr_r[numb_r-1].library[i+2].Id_b == numb_b)
				{
					...
					arr_r[numb_r-1].library[i+2].Flag_b = 0; 	// удаляет книгу из ячейки читателя
					arr_b[numb_b-1].Flag_b = 1;
					//std::cerr << "Break\n";
					break;
				}
				...
				{
					 std::cout << "Not a Book for read" << std::endl;
					break;
				} 
			}
		}
		...
		{
			std::cout << "Reader not a Book for read" << std::endl;
		}
		...
		flag_book.close();
		std::cout << "Return more a book? Y/N: ";
		std::cin.ignore(1000,'\n');
		std::cin.clear();
		std::cin >> ask;
	}
...
void add_book_read (){
	...
	do
	{
		...
		{
			std::cout << "Reader not found. Maybe removed." << std::endl;
		}
		std::ofstream book_add_read ("Reader.bin");
		book_add_read.write(reinterpret_cast<char*>(&arr_r),sizeof(arr_r));
		book_add_read.close();
...
void reader_book_r(){	
	...
	if (read_book)
	{
		...
		if (choice ==2)
		{
			...
			{
				std::cout << arr_r[numb_r-1].Id_r << ' ' << arr_r[numb_r-1].Name_r << std::endl;
				std::cout << "Book for Reader read:"<< std::endl;
				for (int j = 0; j<3; j++)
				{
					if (arr_r[numb_r-1].library[j].Flag_b == 1)
					{
				...
				std::cout << "History Book for Reader read:"<< std::endl;
				for (int i=0, j = 1; i<j; i++)
					{
						if (arr_r[numb_r-1].Hist_book[i].Flag_b == 1)
						{
							std::cout << arr_r[numb_r-1].Hist_book[i].Name_b << std::endl;
							std::cerr << "Hist\n";
							j++;
